...
fn getAvatarMeta(avatar_id: u32) ?AvatarConfig.Avatar {
    for (avatar_config.items) |meta| {
        if (meta.avatar_id == avatar_id) return meta;
    }
    return null;
}
...
pub fn createAllAvatar(
    ...
    var avatar = protocol.Avatar.init(allocator);
    const meta = getAvatarMeta(Avatar_id) orelse return error.AvatarMetaNotFound;
    avatar.base_avatar_id = meta.adventure_player_id;
    avatar.level = 80;
    avatar.promotion = 6;
    avatar.exp = 0;
    avatar.equipment_unique_id = 0;
    avatar.first_met_time_stamp = 0;
    avatar.is_marked = false;
    avatar.has_taken_promotion_reward_list = ArrayList(u32).init(allocator);
    for (1..6) |i| {
        try avatar.has_taken_promotion_reward_list.append(@intCast(i));
...
pub fn createAvatarPathData(
    ...
    avatar.unlock_timestamp = 0; // TODO: actual timestamp
...
fn getAvatarType(id: u32) protocol.MultiPathAvatarType {
    return switch (id) {
        ...
        1224 => .Mar_7thRogueType,
            ...
            if (id < 8001 or id > 8010) return .MultiPathAvatarTypeNone;
