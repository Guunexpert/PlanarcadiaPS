...
//end of program feature
    // Additional standalone run steps
    const dispatch = b.dependency("dispatch", dep_opts);
    b.installArtifact(dispatch.artifact("dispatch"));

    const gameserver_dep = b.dependency("gameserver", dep_opts);
    b.installArtifact(gameserver_dep.artifact("gameserver"));
    ...
    const gameserver_cmd = b.addRunArtifact(gameserver_dep.artifact("gameserver"));
